...
window.GameDirector.observe(T.state, { text: '🏘️ Wohnbezirk Stufe 2.' });
window.GameUI.render();
var directorButtons = Array.prototype.map.call(document.querySelectorAll('#screen .watch-card .btn'), function (button) { return button.textContent; });
ok(directorButtons.some(function (text) { return text.indexOf('Meilenstein') >= 0; }) &&
  directorButtons.some(function (text) { return text.indexOf('Risiko') >= 0; }), 'Watch-Karte bietet ereignisbasierte Zeitraffer');
ok(document.querySelectorAll('#screen .director-entry').length === 1, 'Director-Feed gruppiert Auto-Aktionen');
var decisionBtn = Array.prototype.filter.call(document.querySelectorAll('#screen .watch-card .btn'), function (button) {
  return button.textContent.indexOf('Entscheidung') >= 0;
})[0];
decisionBtn.click();
ok(T.state.settings.watchPauseDecision === false, 'Entscheidungspause ist direkt umschaltbar');
T.state.settings.watchPauseDecision = true; window.GameUI.render();
